...
def accept_arguments():
    ...
    if os.path.isfile(img_path):
        model, size, n_channels = inference.load_model(model_path)
        prediction = inference.show_img_with_seg(img_path, model, size,n_channels, show_results)
    else:
        model, size, n_channels = inference.load_model(model_path)
        inference.batch_prediction(img_path, model, size, n_channels, masked_img_path, masks_path)
